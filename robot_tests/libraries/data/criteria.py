...
from data.criteria_templates import FOLDERS, TEMPLATES
...
# Criteria bound to one item rather than to the tender or a lot.
ITEM_RELATED = tuple(key for key, template in TEMPLATES.items() if template.get("relatesTo") == "item")

# What a criterion can be bound to, and so what ``related_item`` means for it.
BOUND = ("lot", "item")
...
def criterion(classification_id: str, related_item: str | None = None, **kwargs: Any) -> dict[str, Any]:
    ...
    ``related_item`` binds a criterion to the lot or the item it is about;
    without it such a criterion falls back to relating to the tender as a
    whole.
    """
    ...
    if data.get("relatesTo") in BOUND:
        if related_item:
            data["relatedItem"] = related_item
        else:
            # nothing to bind it to, so it is about the tender as a whole
            data["relatesTo"] = "tender"
    return apply_overrides(data, kwargs)


def criteria_for(
    classification_ids_: list[str],
    ...
    plain lot id. ``related_item`` is the item the item related criteria are
    about, for the procedures that have them.
    """
    ...
    for classification_id in classification_ids_:
        ...
        elif classification_id in ITEM_RELATED:
            result.append(criterion(classification_id, related_item=related_item))
        else:
...
DEFAULT_FOLDER = "aboveThreshold"


def folder_criteria(folder: str) -> list[str]:
    """The criteria a data folder posts, in the order it posts them."""
    if folder not in FOLDERS:
        raise ValueError(f"no data folder {folder!r} posts criteria; see criteria_templates.FOLDERS")
    # the folder lists a lot related criterion once per lot, which criteria_for
    # does for itself, so each classification is named once here
    seen: list[str] = []
...
def criteria_data(
    ...
    """
    The criteria payload of a tender, as one data folder posts it.

    Which criteria belong to a procedure is not a matter of taste: the life
    cycle cost ones are refused unless the tender is judged on life cycle cost,
    and the localisation one needs an item from the catalogue. ``folder`` picks
    the set a bundled data folder uses, which is the set that procedure
    accepts. ``include`` overrides it, ``exclude`` drops some of it.
    """
    selected = list(include) if include else folder_criteria(folder)
    if exclude:
        selected = [key for key in selected if key not in set(exclude)]
    return {"data": criteria_for(selected, lots, related_item=related_item)}

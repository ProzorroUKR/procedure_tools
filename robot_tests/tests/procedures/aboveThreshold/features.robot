...
*** Налаштування ***
Документація      Понад поріг, де пропозиції оцінюють не лише за ціною.
...
...               Закупівля оголошує два нецінові критерії - один про лот,
...               один про предмет закупівлі - і вказує, скільки вартий кожен
...               варіант відповіді. Кожна пропозиція мусить відповісти на
...               обидва, а аукціон ранжує їх за ціною і відповідями разом.
...               Беруть участь троє учасників, щоб відповіді справді їх
...               розділяли.
...
...
Тестові теги      procedure:aboveThreshold    feature:features    slow
...
Опублікувати Закупівлю
    [Документація]    Три лоти і два нецінові критерії: один про лот, один
    ...               про предмет закупівлі. Нецінові критерії оголошують разом
    ...               із закупівлею, бо пропозиція не може відповісти на
    ...               критерій, доданий пізніше.
    [Теги]    phase:tendering

    ${lots}=        Дані Лотів    3
    # неціновий критерій вказує на предмет закупівлі, якого стосується, тож
    # предметам потрібні власні id, а не ті, які призначило б API
    ${items}=       Дані Предметів Закупівлі    lots=${lots}    with_id=${True}
    ${features}=    Дані Нецінових Критеріїв    ${lots}[0][id]    ${items}[0][id]
    ${tender}=      Створити Відкриту Закупівлю    lots=${lots}    items=${items}    features=${features}
...
Зібрати Пропозиції
    [Документація]    Троє учасників подають по пропозиції, відповідають на
    ...               критерії й на обидва нецінові критерії. Пропозицію, яка
    ...               залишила неціновий критерій без відповіді, відхилили б.
    [Теги]    phase:tendering
    ...
    FOR    ${bid}    IN    @{bids}
        Length Should Be    ${bid}[parameters]    2
    END
